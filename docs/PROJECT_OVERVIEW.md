...
**Focus Drift** is a privacy-first attention analysis system that detects and explains attention drift during a browsing session by analyzing **interaction behavior**, not content.
...
### We ARE building:
> A behavioral signal processing system that models how attention degrades over time and explains why.
...
- Whether it was caused by interruptions or fragmentation
...
Every design decision must respect the following principles:
...
If a feature violates even **one** of these principles, it does not belong.
...
Every team member must agree to the following:

> “We are building a behavior-aware system that explains attention drift using deterministic metrics and a small trained model, while preserving privacy and explainability.”
